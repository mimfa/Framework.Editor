...
namespace MiMFa.Controls.WinForm.Editor.Tools
{
    ...
    public class EditMap : Control
    {
        ...
        protected virtual void UnSubscribe(EditBox target)
        {
        ...
        protected virtual void Subscribe(EditBox target)
        {
            target.Scroll += new ScrollEventHandler(Target_Scroll);
            target.SelectionChangedDelayed += new EventHandler(Target_SelectionChanged);
            target.VisibleRangeChanged += new EventHandler(Target_VisibleRangeChanged);
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (startPlace.LineIndex > r.Start.LineIndex)
                startPlace.LineIndex = r.Start.LineIndex;
            else
            {
                ...
                if (pp.LineIndex > startPlace.LineIndex)
                    startPlace.LineIndex = pp.LineIndex;
            }
            startPlace.CharIndex = 0;
            //calc scroll pos
            var linesCount = target.Lines.Count;
            var sp1 = (float)r.Start.LineIndex / linesCount;
            var sp2 = (float)r.End.LineIndex / linesCount;
