...
namespace MiMFa.Controls.WinForm.Editor.Model
{
    ...
    public class InsertCharCommand : UndoableCommand
    {
        ...
        public override void Undo()
        {
            ...
            switch (c)
            {
                case '\n': MergeLines(sel.Start.LineIndex, ts); break;
                case '\r': break;
                case '\b':
                    ...
                    if (deletedChar != '\x0')
                    {
                        ts.CurrentTB.ExpandBlock(ts.CurrentTB.Selection.Start.LineIndex);
                        InsertChar(deletedChar, ref cc, ts);
                    }
                    break;
                case '\t':
                    ts.CurrentTB.ExpandBlock(sel.Start.LineIndex);
                    for (int i = sel.FromX; i < lastSel.FromX; i++)
                        ts[sel.Start.LineIndex].RemoveAt(sel.Start.CharIndex);
            ...
            ts.NeedRecalc(new TextSource.TextChangedEventArgs(sel.Start.LineIndex, sel.Start.LineIndex));
        ...
        public override void Execute()
        {
            ...
            ts.NeedRecalc(new TextSource.TextChangedEventArgs(ts.CurrentTB.Selection.Start.LineIndex, ts.CurrentTB.Selection.Start.LineIndex));
            base.Execute();
        }
        ...
        internal static void InsertChar(char c, ref char deletedChar, TextSource ts)
        {
            ...
            switch (c)
            {
                ...
                case '\r': break;
                case '\b'://backspace
                    if (tb.Selection.Start.CharIndex == 0 && tb.Selection.Start.LineIndex == 0)
                        return;
                    if (tb.Selection.Start.CharIndex == 0)
                    {
                        if (!ts.CurrentTB.AllowInsertRemoveLines)
                            throw new ArgumentOutOfRangeException("Cant insert this char in ColumnRange mode");
                        if (tb.LineInfos[tb.Selection.Start.LineIndex - 1].VisibleState != VisibleState.Visible)
                            tb.ExpandBlock(tb.Selection.Start.LineIndex - 1);
                        deletedChar = '\n';
                        MergeLines(tb.Selection.Start.LineIndex - 1, ts);
                    }
                    else
                    {
                        deletedChar = ts[tb.Selection.Start.LineIndex][tb.Selection.Start.CharIndex - 1].c;
                        ts[tb.Selection.Start.LineIndex].RemoveAt(tb.Selection.Start.CharIndex - 1);
                        tb.Selection.Start = new Place(tb.Selection.Start.CharIndex - 1, tb.Selection.Start.LineIndex);
                    }
                    break;
                case '\t':
                    if (ts.CurrentTB.ProfessionalBehavior)
                    {
                        int spaceCountNextTabStop = tb.TabLength - (tb.Selection.Start.CharIndex % tb.TabLength);
                        if (spaceCountNextTabStop == 0)
                            spaceCountNextTabStop = tb.TabLength;

                        for (int i = 0; i < spaceCountNextTabStop; i++)
                            ts[tb.Selection.Start.LineIndex].Insert(tb.Selection.Start.CharIndex, new Char(' '));

                        tb.Selection.Start = new Place(tb.Selection.Start.CharIndex + spaceCountNextTabStop, tb.Selection.Start.LineIndex);
                    }
                    else
                    {
                        ts[tb.Selection.Start.LineIndex].Insert(tb.Selection.Start.CharIndex, new Char(c));
                        tb.Selection.Start = new Place(tb.Selection.Start.CharIndex + 1, tb.Selection.Start.LineIndex);
                    }
                    break;
                default:
        ...
        internal static void InsertLine(TextSource ts)
        {
            ...
            if (ts.Count == 0)
                ...
                BreakLines(tb.Selection.Start.LineIndex, tb.Selection.Start.CharIndex, ts);

            tb.Selection.Start = new Place(0, tb.Selection.Start.LineIndex + 1);
    ...
    public class InsertTextCommand : UndoableCommand
    {
        ...
        internal static void InsertText(string insertedText, TextSource ts)
        {
            ...
            try
            {
                ...
                tb.ExpandBlock(tb.Selection.Start.LineIndex);
    ...
    public class ReplaceTextCommand : UndoableCommand
    {
            ...
            : base(ts)
        {
            ...
            ranges.Sort((r1, r2)=>
                {
                    if (r1.Start.LineIndex == r2.Start.LineIndex)
                        return r1.Start.CharIndex.CompareTo(r2.Start.CharIndex);
                    return r1.Start.LineIndex.CompareTo(r2.Start.LineIndex);
                });
            //
            this.ranges = ranges;
            this.insertedText = insertedText;
        ...
        public override void Undo()
        {
            ...
            if (ranges.Count > 0)
                ts.OnTextChanged(ranges[0].Start.LineIndex, ranges[ranges.Count - 1].End.LineIndex);
    ...
    public class ClearSelectedCommand : UndoableCommand
    {
        ...
        public override void Undo()
        {
            ts.CurrentTB.Selection.Start = new Place(sel.FromX, Math.Min(sel.Start.LineIndex, sel.End.LineIndex));
            ts.OnTextChanging();
            InsertTextCommand.InsertText(deletedText, ts);
            ts.OnTextChanged(sel.Start.LineIndex, sel.End.LineIndex);
            ts.CurrentTB.Selection.Start = sel.Start;
        ...
        public override void Execute()
        {
            ...
            ClearSelected(ts);
            lastSel = new RangeInfo(tb.Selection);
            ts.OnTextChanged(lastSel.Start.LineIndex, lastSel.Start.LineIndex);
        }
    ...
    public class ReplaceMultipleTextCommand : UndoableCommand
    {
            ...
            : base(ts)
        {
            ...
            ranges.Sort((r1, r2) =>
            {
                if (r1.ReplacedRange.Start.LineIndex == r2.ReplacedRange.Start.LineIndex)
                    return r1.ReplacedRange.Start.CharIndex.CompareTo(r2.ReplacedRange.Start.CharIndex);
                return r1.ReplacedRange.Start.LineIndex.CompareTo(r2.ReplacedRange.Start.LineIndex);
            });
            //
            this.ranges = ranges;
            lastSel = sel = new RangeInfo(ts.CurrentTB.Selection);
        ...
        public override void Undo()
        {
            ...
            for (int i = 0; i < ranges.Count; i++)
            {
                ...
                ts.OnTextChanged(ranges[i].ReplacedRange.Start.LineIndex, ranges[i].ReplacedRange.Start.LineIndex);
            }
            tb.Selection.EndUpdate();
        ...
        public override void Execute()
        {
            ...
            for (int i = ranges.Count - 1; i >= 0; i--)
            {
                ...
                ts.OnTextChanged(ranges[i].ReplacedRange.Start.LineIndex, ranges[i].ReplacedRange.End.LineIndex);
            }
            tb.Selection.EndUpdate();
            ts.NeedRecalc(new TextSource.TextChangedEventArgs(0, 1));
    ...
    public class MultiRangeCommand : UndoableCommand
    {
        ...
        public override void Execute()
        {
            ...
            var iChar = -1;
            var iStartLine = prevSelection.Start.LineIndex;
            var iEndLine = prevSelection.End.LineIndex;
        ...
        private void ExecuteInsertTextCommand(ref int iChar, string text)
        {
            ...
            {
                var line = ts.CurrentTB[r.Start.LineIndex];
                ...
                if (!lineIsEmpty)
                {
                    ...
                    if (r.End < r.Start && insertedText!="")
                    {
                        ...
                        insertedText = new string(' ', r.Start.CharIndex - r.End.CharIndex) + insertedText;
                        r.Start = r.End;
                    }
                    ts.CurrentTB.Selection = r;
                    var c = new InsertTextCommand(ts, insertedText);
                    c.Execute();
                    if (ts.CurrentTB.Selection.End.CharIndex > iChar)
                        iChar = ts.CurrentTB.Selection.End.CharIndex;
