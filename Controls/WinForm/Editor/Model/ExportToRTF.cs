...
namespace MiMFa.Controls.WinForm.Editor.Model
{
    ...
    public class ExportToRTF
    {
        ...
        public string GetRtf(EditBox tb)
        {
            this.tb = tb;
        ...
        public string GetRtf(Range r)
        {
            ...
            r.Normalize();
            int currentLine = r.Start.LineIndex;
            styles[currentStyleId] = null;
            colorTable.Clear();
            //
            var lineNumberColor = GetColorTableNumber(r.tb.LineNumberForeColor);
            ...
            {
                Char c = r.tb[p.LineIndex][p.CharIndex];
                ...
                if (p.LineIndex != currentLine)
                {
                    for (int i = currentLine; i < p.LineIndex; i++)
                    {
                        tempSB.AppendLine(@"\line");
                        if (IncludeLineNumbers)
                            tempSB.AppendFormat(@"{{\cf{1} {0}}}\tab", i + 2, lineNumberColor);
                    }
                    currentLine = p.LineIndex;
                }
                switch (c.c)
                {
                    case '\\':
