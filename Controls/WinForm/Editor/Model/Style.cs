...
namespace MiMFa.Controls.WinForm.Editor.Model
{
    ...
    public abstract class Style : IDisposable
    {
        ...
        /// Occurs when user click on StyleVisualMarker joined to this style 
        /// </summary>
        public virtual void OnVisualMarkerClick(EditBox tb, VisualMarkerEventArgs args)
        {
        ...
        /// </summary>
        protected virtual void AddVisualMarker(EditBox tb, StyleVisualMarker marker)
        {
        ...
        {
            return new Size((range.End.CharIndex - range.Start.CharIndex) * range.tb.CharWidth, range.tb.CharHeight);
        }
    ...
    public class TextStyle : Style
    {
        ...
        public override void Draw(Graphics gr, Point position, Range range)
        {
            ...
            if (BackgroundBrush != null)
                gr.FillRectangle(BackgroundBrush, position.X, position.Y, (range.End.CharIndex - range.Start.CharIndex) * range.tb.CharWidth, range.tb.CharHeight);
            //draw chars
            using(var f = new Font(range.tb.Font, FontStyle))
            {
                Line line = range.tb[range.Start.LineIndex];
                ...
                {
                    //IME mode
                    for (int i = range.Start.CharIndex; i < range.End.CharIndex; i++)
                    {
                        SizeF size = EditBox.GetCharSize(f, line[i].c);
    ...
    public class FoldedBlockStyle : TextStyle
    {
        ...
        public override void Draw(Graphics gr, Point position, Range range)
        {
            if (range.End.CharIndex > range.Start.CharIndex)
            {
                ...
                for (int i = range.Start.CharIndex; i < range.End.CharIndex; i++)
                    if (range.tb[range.Start.LineIndex][i].c != ' ')
                        break;
                    else
                        firstNonSpaceSymbolX += range.tb.CharWidth;

                //create marker
                range.tb.AddVisualMarker(new FoldedAreaMarker(range.Start.LineIndex, new Rectangle(firstNonSpaceSymbolX, position.Y, position.X + (range.End.CharIndex - range.Start.CharIndex) * range.tb.CharWidth - firstNonSpaceSymbolX, range.tb.CharHeight)));
            }
            else
            {
                //draw '...'
                ...
                //create marker
                range.tb.AddVisualMarker(new FoldedAreaMarker(range.Start.LineIndex, new Rectangle(range.tb.LeftIndent + 2, position.Y, 2 * range.tb.CharHeight, range.tb.CharHeight)));
            }
        }
    }
    ...
    public class SelectionStyle : Style
    {
        ...
        public override void Draw(Graphics gr, Point position, Range range)
        {
            ...
            if (BackgroundBrush != null)
            {
                ...
                var rect = new Rectangle(position.X, position.Y, (range.End.CharIndex - range.Start.CharIndex) * range.tb.CharWidth, range.tb.CharHeight);
                if (rect.Width == 0)
                    return;
                ...
                if (ForegroundBrush != null)
                {
                    ...
                    var r = new Range(range.tb, range.Start.CharIndex, range.Start.LineIndex,
                                      Math.Min(range.tb[range.End.LineIndex].Count, range.End.CharIndex), range.End.LineIndex);
    ...
    public class MarkerStyle : Style
    {
        ...
        public override void Draw(Graphics gr, Point position, Range range)
        {
            ...
            if (BackgroundBrush != null)
            {
                Rectangle rect = new Rectangle(position.X, position.Y, (range.End.CharIndex - range.Start.CharIndex) * range.tb.CharWidth, range.tb.CharHeight);
                if (rect.Width == 0)
                    return;
                gr.FillRectangle(BackgroundBrush, rect);
            }
