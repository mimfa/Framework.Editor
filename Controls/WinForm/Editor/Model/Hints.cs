...
namespace MiMFa.Controls.WinForm.Editor.Model
{
    ...
    public class Hints : ICollection<Hint>, IDisposable
    {
        ...
        private void LayoutHint(Hint hint)
        {
            if (hint.Inline)
            {
                if (hint.Range.Start.LineIndex < tb.LineInfos.Count - 1)
                    hint.HostPanel.Top = tb.LineInfos[hint.Range.Start.LineIndex + 1].startY - hint.TopPadding - hint.HostPanel.Height - tb.VerticalScroll.Value;
                else
                    hint.HostPanel.Top = tb.TextHeight + tb.Paddings.Top - hint.HostPanel.Height - tb.VerticalScroll.Value;
            }
            else
            {
                if (hint.Range.Start.LineIndex > tb.LinesCount - 1) return;
                if (hint.Range.Start.LineIndex == tb.LinesCount - 1)
                {
                    var y = tb.LineInfos[hint.Range.Start.LineIndex].startY - tb.VerticalScroll.Value + tb.CharHeight;

                    if (y + hint.HostPanel.Height + 1 > tb.ClientRectangle.Bottom)
                    {
                        hint.HostPanel.Top = Math.Max(0, tb.LineInfos[hint.Range.Start.LineIndex].startY - tb.VerticalScroll.Value - hint.HostPanel.Height);
                    }
                    else
                ...
                {
                    hint.HostPanel.Top = tb.LineInfos[hint.Range.Start.LineIndex + 1].startY - tb.VerticalScroll.Value;
                    if (hint.HostPanel.Bottom > tb.ClientRectangle.Bottom)
                        hint.HostPanel.Top = tb.LineInfos[hint.Range.Start.LineIndex + 1].startY - tb.CharHeight - hint.TopPadding - hint.HostPanel.Height - tb.VerticalScroll.Value;
                }
            }
            ...
            else
            {
                ...
                hint.HostPanel.Width = Math.Max(150, Math.Min(Math.Abs(p2.X - p1.X), Math.Abs(tb.Width - tb.LeftIndent * 2)));
                hint.HostPanel.Left = tb.LeftIndent + tb.LeftPadding + hint.Range.Start.CharIndex * tb.CharWidth;
            }
        }
        ...
        public void Add(Hint hint)
        {
            ...
            {
                var li = tb.LineInfos[hint.Range.Start.LineIndex];
                hint.TopPadding = li.bottomPadding;
                li.bottomPadding += hint.HostPanel.Height;
                tb.LineInfos[hint.Range.Start.LineIndex] = li;
                tb.NeedRecalc(true);
            }
            ...
            if (hint.Closeable)
            {
                ...
                l.Text = "✕";
                l.Font = new Font(l.Font.FontFamily,7f);
                l.Padding = new Padding(0);
                l.Width = 15;
                l.AutoSize = true;
                l.Dock = DockStyle.Right;
                l.SendToBack();
                l.Location = hint.HostPanel.Location;// new Point(h.HostPanel.Location.X - l.Width, h.HostPanel.Location.Y - l.Height);
                hint.HostPanel.Controls.Add(l);
                l.Click += (s, a) => Remove(hint);
            }
        ...
        public bool Remove(Hint item)
        {
            ...
            if (tb.Controls.Count != 0)
            {
                tb.Controls.Remove(item.HostPanel);
    ...
    public class Hint
    {
        ...
        private Hint(Range range, Control innerControl, string text, bool inline, bool dock, bool closeable = true)
        {
            this.Range = range;
            this.Inline = inline;
        ...
        public Hint(Range range, string text, bool inline, bool dock, bool closeable = true)
            : this(range, null, text, inline, dock,  closeable)
        {
        }
        ...
        /// <param name="inline">Inlining. If True then hint will moves apart text</param>
        /// <param name="dock">Docking. If True then hint will fill whole line</param>
        public Hint(Range range, Control innerControl, bool inline, bool dock, bool closeable = true)
            : this(range, innerControl, null, inline, dock,  closeable)
        {
        }
