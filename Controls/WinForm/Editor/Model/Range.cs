...
namespace MiMFa.Controls.WinForm.Editor.Model
{
    ...
    public class Range : IEnumerable<Place>
    {
        ...
        /// Constructor
        /// </summary>
        public Range(EditBox tb, int iStartChar, int iStartLine, int iEndChar, int iEndLine)
            : this(tb)
        {
        ...
        /// Constructor
        /// </summary>
        public Range(EditBox tb, Place start, Place end)
            : this(tb)
        {
            this.start = start;
            this.end = end;
        ...
        /// </summary>
        public Range(EditBox tb, int iLine)
            : this(tb)
        {
        ...
        {
            if (place.LineIndex < Math.Min(start.LineIndex, end.LineIndex)) return false;
            if (place.LineIndex > Math.Max(start.LineIndex, end.LineIndex)) return false;
            ...
            if (columnSelectionMode)
            {
                if (place.CharIndex < s.CharIndex || place.CharIndex > e.CharIndex) return false;
            }
            else
            {
                if (place.LineIndex == s.LineIndex && place.CharIndex < s.CharIndex) return false;
                if (place.LineIndex == e.LineIndex && place.CharIndex > e.CharIndex) return false;
            }
        ...
        /// <param name="range"></param>
        /// <returns></returns>
        public Range GetScope(int backwardChars = -1, int backwardLines = -1, int forwardChars = -1, int forwardLines = -1)
        {
            Range r1 = this.Clone();
            r1.Normalize();
            Place newStart = new Place(Math.Max(0, backwardChars < 0?0: r1.Start.CharIndex - backwardChars), Math.Max(0, backwardLines < 0 ? 0 : r1.Start.LineIndex - backwardLines));
            Place newEnd = new Place(
                Math.Min(
                    tb.Lines[Math.Min(tb.LinesCount-1, forwardLines < 0? tb.LinesCount - 1 : r1.End.LineIndex + forwardLines)].Length,
                    forwardChars < 0?999999999: r1.End.CharIndex + forwardChars
                ),
                Math.Min(
                    tb.LinesCount-1,
                    forwardLines < 0 ? tb.LinesCount - 1 : r1.End.LineIndex + forwardLines
                )
            );
        ...
        public virtual string Text
        {
            get
            {
                ...
                int fromLine = Math.Min(end.LineIndex, start.LineIndex);
                int toLine = Math.Max(end.LineIndex, start.LineIndex);
                int fromChar = FromX;
                int toChar = ToX;
        ...
        internal void GetText(out string text, out List<Place> charIndexToPlace)
        {
            ...
            //
            int fromLine = Math.Max(Math.Min(end.LineIndex, start.LineIndex), 0);
            int toLine = Math.Min(Math.Max(end.LineIndex, start.LineIndex), tb.LinesCount-1);
            int fromChar = FromX;
            int toChar = ToX;
        ...
        public char CharAfterStart
        {
            get
            {
                if (Start.CharIndex >= tb[Start.LineIndex].Count)
                    return '\n';
                return tb[Start.LineIndex][Start.CharIndex].c;
            }
        }
        ...
        public char CharBeforeStart
        {
            get
            {
                ...
                if (Start.CharIndex <= 0)
                    return '\n';
                return tb[Start.LineIndex][Start.CharIndex - 1].c;
            }
        }
        ...
        internal int FromX
        {
            get
            {
                if (end.LineIndex < start.LineIndex) return end.CharIndex;
                if (end.LineIndex > start.LineIndex) return start.CharIndex;
                return Math.Min(end.CharIndex, start.CharIndex);
            }
        }
        ...
        internal int ToX
        {
            get
            {
                if (end.LineIndex < start.LineIndex) return start.CharIndex;
                if (end.LineIndex > start.LineIndex) return end.CharIndex;
                return Math.Max(end.CharIndex, start.CharIndex);
            }
        }

        public int FromLine
        {
            get { return Math.Min(Start.LineIndex, End.LineIndex); }
        }

        public int ToLine
        {
            get { return Math.Max(Start.LineIndex, End.LineIndex); }
        }
        ...
        public virtual bool GoRightThroughFolded()
        {
            ...
            if (start.LineIndex >= tb.LinesCount - 1 && start.CharIndex >= tb[tb.LinesCount - 1].Count)
                return false;

            if (start.CharIndex < tb[start.LineIndex].Count)
                start.Offset(1, 0);
            else
                start = new Place(0, start.LineIndex + 1);
        ...
        public bool GoLeftThroughFolded()
        {
            ...
            if (start.CharIndex == 0 && start.LineIndex == 0)
                return false;

            if (start.CharIndex > 0)
                start.Offset(-1, 0);
            else
                start = new Place(tb[start.LineIndex - 1].Count, start.LineIndex - 1);
        ...
        public void GoLeft(bool shift)
        {
            ...
            if (start.CharIndex != 0 || start.LineIndex != 0)
            {
                if (start.CharIndex > 0 && tb.LineInfos[start.LineIndex].VisibleState == VisibleState.Visible)
                    start.Offset(-1, 0);
                else
                {
                    int i = tb.FindPrevVisibleLine(start.LineIndex);
                    if (i == start.LineIndex) return;
        ...
        public void GoRight(bool shift)
        {
            ...
            if (start.LineIndex < tb.LinesCount - 1 || start.CharIndex < tb[tb.LinesCount - 1].Count)
            {
                if (start.CharIndex < tb[start.LineIndex].Count && tb.LineInfos[start.LineIndex].VisibleState == VisibleState.Visible)
                    start.Offset(1, 0);
                else
                {
                    int i = tb.FindNextVisibleLine(start.LineIndex);
                    if (i == start.LineIndex) return;
                    start = new Place(0, i);
                }
            }
        ...
        internal void GoUp(bool shift)
        {
            ...
            if (preferedPos < 0)
                preferedPos = start.CharIndex - tb.LineInfos[start.LineIndex].GetWordWrapStringStartPosition(tb.LineInfos[start.LineIndex].GetWordWrapStringIndex(start.CharIndex));

            int iWW = tb.LineInfos[start.LineIndex].GetWordWrapStringIndex(start.CharIndex);
            if (iWW == 0)
            {
                if (start.LineIndex <= 0) return;
                int i = tb.FindPrevVisibleLine(start.LineIndex);
                if (i == start.LineIndex) return;
                start.LineIndex = i;
                iWW = tb.LineInfos[start.LineIndex].WordWrapStringsCount;
            }

            if (iWW > 0)
            {
                int finish = tb.LineInfos[start.LineIndex].GetWordWrapStringFinishPosition(iWW - 1, tb[start.LineIndex]);
                start.CharIndex = tb.LineInfos[start.LineIndex].GetWordWrapStringStartPosition(iWW - 1) + preferedPos;
                if (start.CharIndex > finish + 1)
                    start.CharIndex = finish + 1;
            }
        ...
        internal void GoPageUp(bool shift)
        {
            ...
            for (int i = 0; i < pageHeight; i++)
            {
                ...
                if (iWW == 0)
                {
                    if (start.LineIndex <= 0) break;
                    //pass hidden
                    int newLine = tb.FindPrevVisibleLine(start.LineIndex);
                    if (newLine == start.LineIndex) break;
                    start.LineIndex = newLine;
                    iWW = tb.LineInfos[start.LineIndex].WordWrapStringsCount;
                }
        ...
        internal void GoDown(bool shift)
        {
            ...
            int iWW = tb.LineInfos[start.LineIndex].GetWordWrapStringIndex(start.CharIndex);
            if (iWW >= tb.LineInfos[start.LineIndex].WordWrapStringsCount - 1)
            {
                if (start.LineIndex >= tb.LinesCount - 1) return;
                //pass hidden
                int i = tb.FindNextVisibleLine(start.LineIndex);
                if (i == start.LineIndex) return;
                start.LineIndex = i;
        ...
        internal void GoPageDown(bool shift)
        {
            ...
            for (int i = 0; i < pageHeight; i++)
            {
                ...
                if (iWW >= tb.LineInfos[start.LineIndex].WordWrapStringsCount - 1)
                {
                    if (start.LineIndex >= tb.LinesCount - 1) break;
                    //pass hidden
                    int newLine = tb.FindNextVisibleLine(start.LineIndex);
                    if (newLine == start.LineIndex) break;
                    start.LineIndex = newLine;
                    iWW = -1;
                }
        ...
        internal void GoHome(bool shift)
        {
            ...
            if (tb.LineInfos[start.LineIndex].VisibleState != VisibleState.Visible)
                return;

            start = new Place(0, start.LineIndex);
        ...
        internal void GoEnd(bool shift)
        {
            ...
            start = new Place(tb[start.LineIndex].Count, start.LineIndex);
        ...
        {
            if (Math.Abs(Start.LineIndex - End.LineIndex) > 1000)
                options |= HighlightingPattern.RegexCompiledOption;
            //
        ...
        {
            SetFoldingMarkers(startFoldingPattern, finishFoldingPattern, HighlightingPattern.RegexCompiledOption);
        }
        ...
        public void SetFoldingMarkers(string startFoldingPattern, string finishFoldingPattern, RegexOptions options)
        {
            ...
            foreach (var range in GetRanges(startFoldingPattern, options))
                tb[range.Start.LineIndex].FoldingStartMarker = startFoldingPattern;

            foreach (var range in GetRanges(finishFoldingPattern, options))
                tb[range.Start.LineIndex].FoldingEndMarker = startFoldingPattern;
            //
            tb.Invalidate();
        }
        ...
        public void SetFoldingMarkers(string foldingPattern, RegexOptions options)
        {
            ...
            {
                if (range.Start.LineIndex > 0)
                    tb[range.Start.LineIndex - 1].FoldingEndMarker = foldingPattern;
                tb[range.Start.LineIndex].FoldingStartMarker = foldingPattern;
            }
        ...
        public IEnumerable<Range> GetRangesByLines(Regex regex)
        {
            ...
            //enumaerate lines
            for (int iLine = Start.LineIndex; iLine <= End.LineIndex; iLine++)
            {
                //
                ...
                if (iLine == Start.LineIndex || iLine == End.LineIndex)
        ...
        {
            return GetRangesByLinesReversed( new Regex(regexPattern, options));
        }
        ...
        /// <param name="regexPattern">Regex pattern</param>
        /// <returns>Enumeration of ranges</returns>
        public IEnumerable<Range> GetRangesByLinesReversed(Regex regex)
        {
            Normalize();
            //
            var fts = tb.TextSource as FileTextSource; //<----!!!! ugly
            ...
            //enumaerate lines
            for (int iLine = End.LineIndex; iLine >= Start.LineIndex; iLine--)
            {
                //
                bool isLineLoaded = fts != null ? fts.IsLineLoaded(iLine) : true;
                //
        ...
        public void Expand()
        {
            Normalize();
            start = new Place(0, start.LineIndex);
            end = new Place(tb.GetLineLength(end.LineIndex), end.LineIndex);
        }
        ...
        public Range GetFragment(string allowedSymbolsPattern, bool left = true, bool right = true)
        {
            return GetFragment(allowedSymbolsPattern, RegexOptions.Multiline, left, right);
        }
        ...
        /// <returns>Range of found fragment</returns>
        public Range GetFragment(Style style, bool allowLineBreaks, bool left = true, bool right = true)
        {
            var mask = tb.GetStyleIndexMask(new Style[] { style });
            //
            Place startFragment = Start;
            Place endFragment = End;
            ...
            if (left)
            {
                ...
                while (r.GoLeftThroughFolded())
                {
                    if (!allowLineBreaks && r.CharAfterStart == '\n')
                        break;
                    if (r.Start.CharIndex < tb.GetLineLength(r.Start.LineIndex))
        ...
        /// <param name="allowedSymbolsPattern">Allowed chars pattern for fragment</param>
        /// <returns>Range of found fragment</returns>
        public Range GetFragment(string allowedSymbolsPattern, RegexOptions options, bool left = true, bool right = true)
        {
            ...
            if (left)
            {
                ...
                do
                    if (regex.Match(r.CharBeforeStart.ToString()).Length < 1)
                        break;
                    else ct = r.CharBeforeStart.ToString() + ct;
                while (r.GoLeftThroughFolded());
                startFragment = r.Start;
                r.Start = start;
            }
            if (right)
            {
                ...
                do
                    if (regex.Match(r.CharAfterStart.ToString()).Length < 1)
                        break;
                    else ct += r.CharAfterStart.ToString();
                while (r.GoRightThroughFolded());
                endFragment = r.Start;
            }
        ...
        internal void GoFirst(bool shift)
        {
            ...
            start = new Place(0, 0);
            if (tb.LineInfos[Start.LineIndex].VisibleState != VisibleState.Visible)
                tb.ExpandBlock(Start.LineIndex);
        ...
        public RangeRect Bounds
        {
            get
            {
                int minX = Math.Min(Start.CharIndex, End.CharIndex);
                int minY = Math.Min(Start.LineIndex, End.LineIndex);
                int maxX = Math.Max(Start.CharIndex, End.CharIndex);
                int maxY = Math.Max(Start.LineIndex, End.LineIndex);
            ...
            get
            {
                ...
                if (readonlyStyle != null)
                {
                    ...
                    if (IsEmpty)
                    {
                        ...
                        var line = tb[start.LineIndex];
                        if (columnSelectionMode)
                        {
                            foreach (var sr in GetSubRanges(false))
                            {
                                line = tb[sr.start.LineIndex];
                                if (sr.start.CharIndex < line.Count && sr.start.CharIndex > 0)
                                {
                                    var left = line[sr.start.CharIndex - 1];
                                    var right = line[sr.start.CharIndex];
                                    if ((left.style & si) != 0 &&
                        ...
                        if (start.CharIndex < line.Count && start.CharIndex > 0)
                        {
                            var left = line[start.CharIndex - 1];
                            var right = line[start.CharIndex];
                            if ((left.style & si) != 0 &&
                                (right.style & si) != 0) return true;//we are between readonly chars
                        }
                    }
        ...
        public bool IsReadOnlyLeftChar()
        {
            ...
            r.Normalize();
            if (r.start.CharIndex == 0) return false;
            if (ColumnSelectionMode)
        ...
        public bool IsReadOnlyRightChar()
        {
            ...
            r.Normalize();
            if (r.end.CharIndex >= tb[end.LineIndex].Count) return false;
            if (ColumnSelectionMode)
        ...
        public IEnumerable<Place> GetPlacesCyclic(Place startPlace, bool backward = false)
        {
            if (backward)
            {
                ...
                {
                    if (r.Start.CharIndex < tb[r.Start.LineIndex].Count)
        ...
        private Range GetIntersectionWith_ColumnSelectionMode(Range range)
        {
            if (range.Start.LineIndex != range.End.LineIndex)
                return new Range(tb, Start, Start);
            var rect = Bounds;
            if (range.Start.LineIndex < rect.iStartLine || range.Start.LineIndex > rect.iEndLine)
                return new Range(tb, Start, Start);

            return new Range(tb, rect.iStartChar, range.Start.LineIndex, rect.iEndChar, range.Start.LineIndex).GetIntersectionWith(range);
        }
        ...
        {
            var iLine = tb.FindNextVisibleLine(End.LineIndex);
            End = new Place(End.CharIndex, iLine);
        }

        internal void GoUp_ColumnSelectionMode()
        {
            var iLine = tb.FindPrevVisibleLine(End.LineIndex);
            End = new Place(End.CharIndex, iLine);
        }

        internal void GoRight_ColumnSelectionMode()
        {
            End = new Place(End.CharIndex + 1, End.LineIndex);
        }
