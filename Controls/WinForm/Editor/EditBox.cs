...
namespace MiMFa.Controls.WinForm.Editor
{
    ...
    public partial class EditBox : UserControl, ISupportInitialize
    {
        ...
        private readonly Timer ChangeTimer = new Timer();
        private readonly Timer TextChangeTimer = new Timer();
        ...
        private System.DateTime lastNavigatedDateTime;
        ...
        public EditBox()
        {
            ...
            LineNumberForeColor = Color.Teal;
            IndentBackColor = Color.WhiteSmoke;
            ServiceLinesColor = Color.Silver;
            FoldingIndicatorColor = Color.Green;
            CurrentLineBackColor = Color.Transparent;
            ChangedLineBackColor = Color.Transparent;
            ...
            needRecalc = true;
            lastNavigatedDateTime = System.DateTime.Now;
            AutoIndent = true;
            ...
            AllowDrop = true;
            FindEndOfFoldingBlockStrategy = BlockStrategy.Classic;
            VirtualSpace = false;
            ...
            AutoIndentPattern = @"^\s*[\w\.]+(\s\w+)?\s*(?<range>=)\s*(?<range>[^;=]+);
^\s*(case|default)\s*[^:]*(?<range>:)\s*(?<range>[^;]+);";
        ...
        public void Init()
        {
            ...
            ResetTimer(ChangeTimer);
            ResetTimer(TextChangeTimer);
        }

        private char[] autoCompleteBracketsList = { '(', ')', '{', '}', '[', ']', '"', '"', '\'', '\'' };

        public bool ProfessionalBehavior { get; set; } = true;
        ...
        /// </summary>
        [DefaultValue(typeof(BlockStrategy), "Strategy1")]
        [Description("Strategy of search of brackets to highlighting.")]
        public BlockStrategy BracketsHighlightStrategy { get; set; }
        ...
        /// </summary>
        [DefaultValue(BlockStrategy.Classic)]
        [Description("Strategy of search of end of folding block.")]
        public BlockStrategy FindEndOfFoldingBlockStrategy { get; set; }
        ...
        public Color ChangedLineBackColor
        {
        ...
        public char LeftBracket1 { get; set; }
        ...
        public char RightBracket1 { get; set; }
        ...
        public bool IsReplaceMode
        {
            get
            {
                return isReplaceMode && 
                       ...
                       Selection.Start.CharIndex < lines[Selection.Start.LineIndex].Count;
            }
        ...
        {
            get { return TextChangeTimer.Interval; }
            set { TextChangeTimer.Interval = value; }
        }
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public Highlighter SyntaxHighlighter { get; set; } = new Highlighter();
        ...
        public string SyntaxMapXMLFile
        {
            get { return SyntaxHighlighter.MapXMLFile; }
            set
            {
                SyntaxHighlighter.MapXMLFile = value;
                Invalidate();
            }
        }
        ...
        /// </summary>
        public Model.Char this[Place place]
        {
            get { return lines[place.LineIndex][place.CharIndex]; }
            set { lines[place.LineIndex][place.CharIndex] = value; }
        }
        ...
        {
            get { return Math.Max(PlaceToPosition(Selection.Start), PlaceToPosition(Selection.End)); }
            set { Selection.End = PositionToPlace(value); }
        }
        ...
        /// <summary>
        /// Start place of selection
        /// </summary>
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public Place SelectionStartPlace
        {
            get { return (PlaceToPosition(Selection.Start)< PlaceToPosition(Selection.End))? Selection.Start: Selection.End; }
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public Place SelectionEndPlace
        {
            get { return (PlaceToPosition(Selection.Start) > PlaceToPosition(Selection.End)) ? Selection.Start : Selection.End; }
        ...
        public virtual string HoveredWordPattern { get; set; } = "\\w|\\.";
        protected virtual void OnToolTip()
        {
            ...
            string hoveredWord = r.GetFragment(HoveredWordPattern).Text;
            //event handler
        ...
        public List<Style> GetStylesOfChar(Place place)
        {
            ...
            if (place.LineIndex < LinesCount && place.CharIndex < this[place.LineIndex].Count)
            {
#if Styles32
        ...
        {
            System.DateTime min = System.DateTime.Now;
            int iLine = -1;
        ...
        private void TextChangeTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (needRiseTextChangedDelayed)
            {
                ...
                OnSyntaxHighlight(new TextChangedEventArgs(delayedTextChangedRange));
                OnTextChangedDelayed(delayedTextChangedRange);
                delayedTextChangedRange = null;
            }
            try { DelayedTextChanged(delayedTextChangedRange); } catch { }
        }
        ...
        private void ChangeTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (needRiseSelectionChangedDelayed)
            {
                ...
                try { DelayedSelectionChanged(delayedTextChangedRange); } catch { }
            }
        ...
        {
            RecalcScrollByOneLine(Selection.Start.LineIndex);
            //highlight brackets
            ClearBracketsPositions();
            if (LeftBracket1 != '\x0' && RightBracket1 != '\x0')
                HighlightBrackets(LeftBracket1, RightBracket1, ref leftBracketPosition, ref rightBracketPosition);
            ...
            if (Selection.IsEmpty && Selection.Start.LineIndex < LinesCount)
            {
                if (lastNavigatedDateTime != lines[Selection.Start.LineIndex].LastVisit)
                {
                    lines[Selection.Start.LineIndex].LastVisit = System.DateTime.Now;
                    lastNavigatedDateTime = lines[Selection.Start.LineIndex].LastVisit;
                }
            }
        ...
        public virtual Range Find(string pattern, Place findStartPlace, RegexOptions opt = RegexOptions.None) => FindForward(pattern, findStartPlace, opt);
        public virtual Range Find(Regex pattern, Place findStartPlace) => FindForward(pattern, findStartPlace);
        public virtual IEnumerable<Range> FindAll(Regex pattern)
        {
            var range = Range.Clone();//Selection.IsEmpty ? Range.Clone() : Selection.Clone();
            foreach (var r in range.GetRangesByLines(pattern))
                yield return r;
        }
        public virtual IEnumerable<Range> FindAll(string pattern, RegexOptions opt = RegexOptions.None)
        {
            var range = Range.Clone();//Selection.IsEmpty ? Range.Clone() : Selection.Clone();
            foreach (var r in range.GetRangesByLines(pattern, opt))
                yield return r;
        }
        public virtual Range FindBackward(string pattern, Place findStartPlace, RegexOptions opt = RegexOptions.None) => FindBackward(new Regex(pattern, opt), findStartPlace);
        public virtual Range FindBackward(Regex pattern, Place findStartPlace)
        {
            var range = GetRange(new Place(0,0), findStartPlace);
            foreach (var r in range.GetRangesByLinesReversed(pattern))
            {
                Selection.BeginUpdate();
                SetSelectedLine(r.ToLine);
                Selection = r;
                Invalidate();
                Selection.EndUpdate();
                return r;
        ...
        public virtual Range ReplaceBackward(string pattern, string replacement, Place findStartPlace, RegexOptions opt = RegexOptions.None) => ReplaceBackward(new Regex(pattern, opt), replacement, findStartPlace);
        public virtual Range ReplaceBackward(Regex pattern, string replacement, Place findStartPlace)
        {
            FindBackward(pattern, findStartPlace);
            var range = Selection;
        ...
        public virtual Range FindForward(string pattern, Place findStartPlace, RegexOptions opt = RegexOptions.None) => FindForward(new Regex(pattern,opt), findStartPlace);
        public virtual Range FindForward(Regex pattern, Place findStartPlace)
        {
            var range = GetRange(findStartPlace, PositionToPlace(TextLength));
            foreach (var r in range.GetRangesByLines(pattern))
            {
                Selection.BeginUpdate();
                SetSelectedLine(r.ToLine);
        ...
        public virtual Range ReplaceForward(string pattern, string replacement, Place findStartPlace, RegexOptions opt = RegexOptions.None) => ReplaceForward(new Regex(pattern, opt), replacement, findStartPlace);
        public virtual Range ReplaceForward(Regex pattern, string replacement, Place findStartPlace)
        {
            FindForward(pattern, findStartPlace);
            var range = Selection;
            if (SelectionLength != 0)
                if (!Selection.ReadOnly)
                    InsertText(replacement);
        ...
        public virtual Range Replace(string replacement)
        {
            var range = Selection;
            if (SelectionLength != 0)
                if (!Selection.ReadOnly)
        ...
        public virtual IEnumerable<Range> ReplaceAll(string pattern, string replacement, RegexOptions opt = RegexOptions.None) => ReplaceAll(new Regex(pattern, opt), replacement);
        public virtual IEnumerable<Range> ReplaceAll(Regex pattern, string replacement)
        {
            Selection.BeginUpdate();
            //search
            var ranges = FindAll(pattern).ToList();
            //check readonly
            var ro = false;
            ...
            if (!ro)
                if (ranges.Count > 0)
                {
                    TextSource.Manager.ExecuteCommand(new ReplaceTextCommand(TextSource, ranges, replacement));
        ...
        /// Shows find dialog
        /// </summary>
        public virtual void ShowFindDialog(string findText = null)
        {
            if (FindForm == null)
            ...
            else if (!Selection.IsEmpty && Selection.Start.LineIndex == Selection.End.LineIndex)
        ...
        /// </summary>
        public virtual void ShowReplaceDialog(string findText = null, string replaceText = null)
        {
            if (ReadOnly)
                return;
            if (ReplaceForm == null)
            ...
            if(replaceText != null)
                ReplaceForm.tbReplace.Text = replaceText;
        ...
        public ReadOnlyStyle ReadOnlyStyle = new ReadOnlyStyle();
        public virtual void Lock()
        {
            if (Selection.IsEmpty) SelectLine();
            Selection.SetStyle(ReadOnlyStyle, Selection.Text);
        }
        public virtual void UnLock()
        {
            if (Selection.IsEmpty) SelectLine();
            Selection.ClearStyle(ReadOnlyStyle);
        }
        /// <summary>
        /// Select all chars of line
        /// </summary>
        ...
        public virtual void InsertText(string text, bool jumpToCaret)
        {
            ...
            try
            {
                ...
                if (Selection.IsEmpty && Selection.Start.CharIndex > GetLineLength(Selection.Start.LineIndex) && VirtualSpace)
                    InsertVirtualSpaces();
        ...
        {
            int lineLength = GetLineLength(Selection.Start.LineIndex);
            int count = Selection.Start.CharIndex - lineLength;
            Selection.BeginUpdate();
            try
            {
                Selection.Start = new Place(lineLength, Selection.Start.LineIndex);
        ...
        /// <summary>
        /// Clear selected or current line
        /// </summary>
        ...
        public Line CurrentLine
        {
            get
            {
                if (Selection.Start.LineIndex == 0) return TextSource[Selection.Start.LineIndex];
                if (Selection.Start.LineIndex > 0) Selection.GoRightThroughFolded();
                return null;
            }
        } 
        /// <summary>
        ...
        public void ClearCurrentLine()
        {
            ...
            lines.Manager.ExecuteCommand(new ClearSelectedCommand(TextSource));
            if (Selection.Start.LineIndex == 0)
        ...
        {
            bool b = base.Focus();
            if(this.HasSourceTextBox) this.SourceTextBox.Focus();
            return b;
        }
        ...
        {
            if (LineInfos[Selection.End.LineIndex].VisibleState != VisibleState.Visible)
                ExpandBlock(Selection.End.LineIndex);

            if (LineInfos[Selection.Start.LineIndex].VisibleState != VisibleState.Visible)
                ExpandBlock(Selection.Start.LineIndex);

            Recalc();
            DoVisibleRectangle(new Rectangle(PlaceToPoint(new Place(0, Selection.End.LineIndex)),
        ...
        public void DoRangeVisible(Range range, bool tryToCentre)
        {
            range = range.Clone();
            range.Normalize();
            range.End = new Place(range.End.CharIndex,
                                  Math.Min(range.End.LineIndex, range.Start.LineIndex + ClientSize.Height/CharHeight));

            if (LineInfos[range.End.LineIndex].VisibleState != VisibleState.Visible)
                ExpandBlock(range.End.LineIndex);

            if (LineInfos[range.Start.LineIndex].VisibleState != VisibleState.Visible)
                ExpandBlock(range.Start.LineIndex);

            Recalc();
            int h = (1 + range.End.LineIndex - range.Start.LineIndex)*CharHeight;
            Point p = PlaceToPoint(new Place(0, range.Start.LineIndex));
            if (tryToCentre)
            {
            ...
            switch (action)
            {
                ...
                case FCTBAction.IndentDecrease:
                    if (!Selection.ReadOnly)
                    {
                        ...
                        {
                            var line = this[sel.Start.LineIndex];
                            if (sel.Start.CharIndex == 0 && sel.End.CharIndex == line.Count)
                                Selection = new Range(this, line.StartSpacesCount, sel.Start.LineIndex, line.Count, sel.Start.LineIndex);
                            else
                            if (sel.Start.CharIndex == line.Count && sel.End.CharIndex == 0)
                                Selection = new Range(this, line.Count, sel.Start.LineIndex, line.StartSpacesCount, sel.Start.LineIndex);
                        }
                ...
                case FCTBAction.IndentIncrease:
                    if (!Selection.ReadOnly)
                    {
                        ...
                        sel.Normalize();
                        var spaces = this[sel.Start.LineIndex].StartSpacesCount;
                        if (sel.Start.LineIndex != sel.End.LineIndex || //selected several lines
                           (sel.Start.CharIndex <= spaces && sel.End.CharIndex == this[sel.Start.LineIndex].Count) || //selected whole line
                           sel.End.CharIndex <= spaces)//selected space prefix
                        {
                            IncreaseIndent();
                            if (sel.Start.LineIndex == sel.End.LineIndex && !sel.IsEmpty)
                            {
                                Selection = new Range(this, this[sel.Start.LineIndex].StartSpacesCount, sel.End.LineIndex, this[sel.Start.LineIndex].Count, sel.End.LineIndex); //select whole line
                                if (inverted)
                                    Selection.Inverse();
                            }
                        }
                ...
                case FCTBAction.AutoIndentChars:
                    if (!Selection.ReadOnly)
                        DoAutoIndentChars(Selection.Start.LineIndex);
                    break;
                ...
                case FCTBAction.UnbookmarkLine:
                    UnbookmarkLine(Selection.Start.LineIndex);
                    break;

                case FCTBAction.BookmarkLine:
                    BookmarkLine(Selection.Start.LineIndex);
                    break;

                case FCTBAction.GoNextBookmark:
                    GotoNextBookmark(Selection.Start.LineIndex);
                    break;

                case FCTBAction.GoPrevBookmark:
                    GotoPrevBookmark(Selection.Start.LineIndex);
                    break;
                ...
                case FCTBAction.DeleteCharRight:
                    if (!Selection.ReadOnly)
                    {
                        ...
                        else
                        {
                            ...
                            if (this[Selection.Start.LineIndex].StartSpacesCount == this[Selection.Start.LineIndex].Count)
                                RemoveSpacesAfterCaret();

                            if (!Selection.IsReadOnlyRightChar())
                                if (Selection.GoRightThroughFolded())
                                {
                                    int iLine = Selection.Start.LineIndex;

                                    InsertChar('\b');

                                    //if removed \n then trim spaces
                                    if (iLine != Selection.Start.LineIndex && AutoIndent)
                                        if (Selection.Start.CharIndex > 0)
                                            RemoveSpacesAfterCaret();
                                }
                        }
        ...
        public virtual void MoveSelectedLinesDown()
        {
            ...
            {
                int iLine = Selection.Start.LineIndex;
                if (Selection.End.LineIndex >= LinesCount - 1)
                {
                ...
                for (int i = Selection.Start.LineIndex; i <= Selection.End.LineIndex; i++)
                    temp.Add(i);
                RemoveLines(temp);
                Selection.Start = new Place(GetLineLength(iLine), iLine);
                SelectedText = "\n" + text;
                Selection.Start = new Place(prevSelection.Start.CharIndex, prevSelection.Start.LineIndex + 1);
                Selection.End = new Place(prevSelection.End.CharIndex, prevSelection.End.LineIndex + 1);
            }else
                Selection = prevSelection;
        }
        ...
        private void GoHome(bool shift)
        {
            ...
            try
            {
                ...
                if (Selection.Start.CharIndex <= spaces)
        ...
        {
            bool b = selection.IsEmpty;
            if (b) Selection.Expand();
        ...
        public string AutoIndentPattern { get; set; }
        ...
        {
            var patterns = AutoIndentPattern.Split(new char[] {'\r', '\n'}, StringSplitOptions.RemoveEmptyEntries);
        ...
        protected void DoAutoIndentChars(int iLine, Regex regex)
        {
            ...
            for (int iCapture = maxCapturesCount - 1; iCapture >= 0; iCapture--)
            {
                ...
                foreach(var i in new List<int>(texts.Keys))
                {
                    ...
                    if (oldSel.Start.LineIndex == i && oldSel.Start.CharIndex > cap.Index)
                        oldSel.Start = new Place(oldSel.Start.CharIndex + addSpaces, i);
        ...
        private bool DoAutocompleteBrackets(char c)
        {
            if (AutoCompleteBrackets)
            {
                ...
                if (!char.IsLetterOrDigit((Selection.GetFragment(".*", false, true).Text.TrimStart() + " ")[0]))
        ...
        private bool InsertBrackets(char left, char right)
        {
            if (Selection.ColumnSelectionMode)
            {
                ...
                Selection.BeginUpdate();
                BeginAutoUndo();
                Selection = new Range(this, range.Start.CharIndex, range.Start.LineIndex, range.Start.CharIndex, range.End.LineIndex) { ColumnSelectionMode = true };
                InsertChar(left);
                Selection = new Range(this, range.End.CharIndex + 1, range.Start.LineIndex, range.End.CharIndex + 1, range.End.LineIndex) { ColumnSelectionMode = true };
                InsertChar(right);
                if (range.IsEmpty)
                    Selection = new Range(this, range.End.CharIndex + 1, range.Start.LineIndex, range.End.CharIndex + 1, range.End.LineIndex) { ColumnSelectionMode = true };
                EndAutoUndo();
        ...
        public virtual void DoAutoIndentIfNeed()
        {
            ...
            {
                DoCaretVisible();
                int needSpaces = CalcAutoIndent(Selection.Start.LineIndex);
                if (this[Selection.Start.LineIndex].AutoIndentSpacesNeededCount != needSpaces)
                {
                    DoAutoIndent(Selection.Start.LineIndex);
                    this[Selection.Start.LineIndex].AutoIndentSpacesNeededCount = needSpaces;
                }
            }
        }
        ...
        public virtual void DoAutoIndent(int iLine)
        {
            ...
            Selection.Start = new Place(Math.Min(lines[iLine].Count, Math.Max(0, oldStart.CharIndex + needToInsert)), iLine);
        }
        ...
        public void DrawText(Graphics gr, Place start, Size size)
        {
            ...
            int firstChar = start.CharIndex;
            int lastChar = (startX + size.Width) / CharWidth;

            var startLine = start.LineIndex;
            //draw text
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Brush changedLineBrush = new SolidBrush(ChangedLineBackColor);
            Brush indentBrush = new SolidBrush(IndentBackColor);
            Brush paddingBrush = new SolidBrush(PaddingBackColor);
            Brush currentLineBrush =
                new SolidBrush(Color.FromArgb(CurrentLineBackColor.A == 255 ? 50 : CurrentLineBackColor.A, CurrentLineBackColor));
            //draw padding area
            ...
            for (iLine = startLine; iLine < lines.Count; iLine++)
            {
                ...
                if (CurrentLineBackColor != Color.Transparent && iLine == Selection.Start.LineIndex)
                    if (Selection.IsEmpty)
                ...
                if (ChangedLineBackColor != Color.Transparent && line.IsChanged)
                ...
                if (ShowLineNumbers)
                {
                    ...
                    using (var lineNumberBrush = new SolidBrush(LineNumberForeColor))
        ...
        {
            if (ServiceColors == null)  ServiceColors= new ServiceColors();
        ...
        private void DrawRecordingHint(Graphics graphics)
        {
            ...
            var ts = new TimeSpan(System.DateTime.Now.Ticks);
            graphics.RotateTransform(180 * (System.DateTime.Now.Millisecond/1000f));
        ...
        private void PaintHintBrackets(Graphics gr)
        {
            foreach (Hint hint in hints)
            {
                ...
                if (GetVisibleState(r.Start.LineIndex) != VisibleState.Visible ||
                    GetVisibleState(r.End.LineIndex) != VisibleState.Visible)
                    continue;
                                   ...
                                   int y)
        {
            ...
            if (!Selection.IsEmpty && lastChar >= firstChar)
            {
                ...
                {
                    SelectionStyle.Draw(gr, new Point(startX + (textRange.Start.CharIndex - from)*CharWidth, 1 + y),
                                        textRange);
                }
            }
        }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (!isLineSelect)
                {
                    ...
                    if (Selection.IsEmpty || !Selection.Contains(p) || this[p.LineIndex].Count <= p.CharIndex || ReadOnly)
                        OnMouseClickText(e);
                    else
                    {
                ...
                else
                {
                    ...
                    Selection.BeginUpdate();
                    //select whole line
                    int iLine = PointToPlaceSimple(e.Location).LineIndex;
        ...
        {
            int fromX = p.CharIndex;
            int toX = p.CharIndex;

            for (int i = p.CharIndex; i < lines[p.LineIndex].Count; i++)
            {
                char c = lines[p.LineIndex][i].c;
            ...
            Selection = new Range(this, toX, p.LineIndex, fromX, p.LineIndex);
        }
        ...
        protected virtual void OnTextChanged(TextChangedEventArgs args)
        {
            ...
            if (updating > 0)
            {
                ...
                {
                    if (updatingRange.Start.LineIndex > args.ChangedRange.Start.LineIndex)
                        updatingRange.Start = new Place(0, args.ChangedRange.Start.LineIndex);
                    if (updatingRange.End.LineIndex < args.ChangedRange.End.LineIndex)
                        updatingRange.End = new Place(lines[args.ChangedRange.End.LineIndex].Count,
                                                      args.ChangedRange.End.LineIndex);
            ...
            //
            if (wordWrap)
                RecalcWordWrap(args.ChangedRange.Start.LineIndex, args.ChangedRange.End.LineIndex);
            //
        ...
        {
            for (int iLine = range.Start.LineIndex; iLine <= range.End.LineIndex; iLine++)
                if (iLine >= 0 && iLine < lines.Count)
        ...
        private void HighlightFoldings()
        {
            ...
            endFoldingLine = -1;
            int counter = 0;
            for (int i = Selection.Start.LineIndex; i >= Math.Max(Selection.Start.LineIndex - maxLinesForFolding, 0); i--)
            {
                ...
                if (hasEndMarker && i != Selection.Start.LineIndex)
                    counter++;
            }
        ...
        public int TryPlaceToPosition(Place point)
        {
            if (point.LineIndex < 0) point = new Place(point.CharIndex, 0);
            if (point.LineIndex >= lines.Count) point = new Place(point.CharIndex, lines.Count-1);
            if (point.CharIndex < 0) point = new Place(0, point.LineIndex);
            if (point.CharIndex >= lines[point.LineIndex].Count + Environment.NewLine.Length) point = new Place(lines[point.LineIndex].Count + Environment.NewLine.Length - 1, point.LineIndex);

            int result = 0;
            for (int i = 0; i < point.LineIndex; i++)
        ...
        {
            if (point.LineIndex < 0 || point.LineIndex >= lines.Count ||
                point.CharIndex >= lines[point.LineIndex].Count + Environment.NewLine.Length)
                return -1;
        ...
        {
            if (place.LineIndex >= LineInfos.Count)
                return new Point();
            int y = LineInfos[place.LineIndex].startY;
            //
            int iWordWrapIndex = LineInfos[place.LineIndex].GetWordWrapStringIndex(place.CharIndex);
            y += iWordWrapIndex*CharHeight;
            int x = (place.CharIndex - LineInfos[place.LineIndex].GetWordWrapStringStartPosition(iWordWrapIndex))*CharWidth;
            if(iWordWrapIndex > 0 )
                x += LineInfos[place.LineIndex].wordWrapIndent * CharWidth;
            //
        ...
        public string GetLineText(int iLine)
        {
            ...
            foreach (Model.Char c in lines[iLine])
                sb.Append(c.c);
            return sb.ToString();
        }
        ...
        protected virtual void RecalcFoldingLines()
        {
            ...
            int startLine = Math.Max(range.Start.LineIndex - maxLinesForFolding, 0);
            int endLine = Math.Min(range.End.LineIndex + maxLinesForFolding, Math.Max(range.End.LineIndex, LinesCount - 1));
        ...
        public virtual void IncreaseIndent()
        {
            if (Selection.Start == Selection.End)
            {
                if (!Selection.ReadOnly)
                {
                    Selection.Start = new Place(this[Selection.Start.LineIndex].StartSpacesCount, Selection.Start.LineIndex);
                    //insert tab as spaces
                    int spaces = TabLength - (Selection.Start.CharIndex % TabLength);
            ...
            if (Selection.ColumnSelectionMode)
                startChar = Math.Min(Selection.End.CharIndex, Selection.Start.CharIndex);
            ...
            int from = Selection.Start.LineIndex;
            int to = Selection.End.LineIndex;

            if (!Selection.ColumnSelectionMode)
                if (Selection.End.CharIndex == 0) to--;
            ...
            {
                int newSelectionStartCharacterIndex = currentSelection.Start.CharIndex + this.TabLength;
                int newSelectionEndCharacterIndex = currentSelection.End.CharIndex + (currentSelection.End.LineIndex == to?this.TabLength : 0);
                this.Selection.Start = new Place(newSelectionStartCharacterIndex, currentSelection.Start.LineIndex);
                this.Selection.End = new Place(newSelectionEndCharacterIndex, currentSelection.End.LineIndex);
            }
            else
            {
                Selection = old;
        ...
        {
            if (Selection.Start.LineIndex == Selection.End.LineIndex)
            {
            ...
            if (Selection.ColumnSelectionMode)
                startCharIndex = Math.Min(Selection.End.CharIndex, Selection.Start.CharIndex);
            ...
            for (int i = from; i <= to; i++)
            {
                ...
                if (i == currentSelection.Start.LineIndex)
                {
                    numberOfDeletedWhitespacesOfFirstLine = numberOfWhitespacesToRemove;
                }
                if (i == currentSelection.End.LineIndex)
                {
            ...
            if (Selection.ColumnSelectionMode == false)
            {
                int newSelectionStartCharacterIndex = Math.Max(0, currentSelection.Start.CharIndex - numberOfDeletedWhitespacesOfFirstLine);
                int newSelectionEndCharacterIndex = Math.Max(0, currentSelection.End.CharIndex - numberOfDeletetWhitespacesOfLastLine);
                this.Selection.Start = new Place(newSelectionStartCharacterIndex, currentSelection.Start.LineIndex);
                this.Selection.End = new Place(newSelectionEndCharacterIndex, currentSelection.End.LineIndex);
            }
            else
        ...
        protected virtual void DecreaseIndentOfSingleLine()
        {
            ...
            Range currentSelection = this.Selection.Clone();
            int currentLineIndex = this.Selection.Start.LineIndex;
            int currentLeftSelectionStartIndex = Math.Min(this.Selection.Start.CharIndex, this.Selection.End.CharIndex);
            ...
            if (numberOfCharactersToRemove > 0)
            {
                ...
                // Restore selection
                int newSelectionStartCharacterIndex = currentSelection.Start.CharIndex - numberOfCharactersToRemove;
                int newSelectionEndCharacterIndex = currentSelection.End.CharIndex - numberOfCharactersToRemove;
        ...
        public virtual void DoAutoIndent()
        {
            ...
            lines.Manager.BeginAutoUndoCommands();
            //
            for (int i = r.Start.LineIndex; i <= r.End.LineIndex; i++)
                DoAutoIndent(i);
            //
            lines.Manager.EndAutoUndoCommands();
        ...
        private void HighlightBrackets(char LeftBracket, char RightBracket, ref Range leftBracketPosition, ref Range rightBracketPosition)
        {
            ...
            {
                case BlockStrategy.Classic: HighlightBrackets1(LeftBracket, RightBracket, ref leftBracketPosition, ref rightBracketPosition); break;
                case BlockStrategy.Modern: HighlightBrackets2(LeftBracket, RightBracket, ref leftBracketPosition, ref rightBracketPosition); break;
            }
        }
        ...
        private void HighlightBrackets1(char LeftBracket, char RightBracket, ref Range leftBracketPosition, ref Range rightBracketPosition)
        {
            ...
            if(range != null)
            {
                leftBracketPosition = new Range(this, range.Start, new Place(range.Start.CharIndex + 1, range.Start.LineIndex));
                rightBracketPosition = new Range(this, new Place(range.End.CharIndex - 1, range.End.LineIndex), range.End);
            }
        ...
        public Range GetBracketsRange(Place placeInsideBrackets, char leftBracket, char rightBracket, bool includeBrackets)
        {
            ...
            while (range.GoLeftThroughFolded()) //move caret left
            {
                ...
                if (counter == 1)
                {
                    range.Start = new Place(range.Start.CharIndex + (!includeBrackets ? 1 : 0), range.Start.LineIndex);
            ...
            do
            {
                ...
                if (counter == -1)
                {
                    range.End = new Place(range.Start.CharIndex + (includeBrackets ? 1 : 0 ), range.Start.LineIndex);
        ...
        private void HighlightBrackets2(char LeftBracket, char RightBracket, ref Range leftBracketPosition, ref Range rightBracketPosition)
        {
            ...
            {
                rightBracketPosition = new Range(this, range.Start.CharIndex - 1, range.Start.LineIndex, range.Start.CharIndex, range.Start.LineIndex);
                while (range.GoLeftThroughFolded()) //move caret left
                {
                    if (range.CharAfterStart == LeftBracket) counter++;
                    if (range.CharAfterStart == RightBracket) counter--;
                    ...
                    {
                        //highlighting
                        range.End = new Place(range.Start.CharIndex + 1, range.Start.LineIndex);
                        leftBracketPosition = range;
                        found = true;
                        break;
                    }
            ...
            {
                leftBracketPosition = new Range(this, range.Start.CharIndex, range.Start.LineIndex, range.Start.CharIndex + 1, range.Start.LineIndex);
                do
                {
                    if (range.CharAfterStart == LeftBracket) counter++;
                    if (range.CharAfterStart == RightBracket) counter--;
        ...
        public virtual void OnSyntaxHighlight(TextChangedEventArgs args)
        {
            ...
            switch (HighlightingRangeType)
            {
                case HighlightingRangeType.VisibleRange:
                    ...
                    break;
                case HighlightingRangeType.ScopeRange:
                    range = args.ChangedRange.GetScope(-1,300,-1,300);
                    break;
            ...
            if (SyntaxHighlighter != null)
                    SyntaxHighlighter.Highlight(range);
        ...
        private string SelectHTMLRangeScript()
        {
            ...
            sel.Normalize();
            int start = PlaceToPosition(sel.Start) - sel.Start.LineIndex;
            int len = sel.Text.Length - (sel.End.LineIndex - sel.Start.LineIndex);
...
window.status = ""#print"";
        ...
        public GoToForm GoToDialog = null;
        /// <summary>
        /// Shows Goto dialog form
        /// </summary>
        public void ShowGoToDialog(int line = -1)
        {
            if(GoToDialog == null) GoToDialog = new GoToForm();
            GoToDialog.TotalLineCount = LinesCount;
            if (line > 0) GoToDialog.SelectedLineNumber = line;
            else GoToDialog.SelectedLineNumber = Selection.Start.LineIndex + 1;

            if (GoToDialog.ShowDialog() == DialogResult.OK)
            {
                SetSelectedLine(GoToDialog.SelectedLineNumber);
            }
        }
        ...
        public void CurrentLineFlashing(Color backColor, int milisecond = 1000, int breaking = 100)
        {
            Color _FlashingMainColor = CurrentLineBackColor;
            int _FlashingIndex = 0;
            Timer _FlashingTimer = new Timer();
            _FlashingTimer.Tick += (s, e) =>
            {
                if (_FlashingIndex * breaking > milisecond)
                {
                    _FlashingTimer.Enabled = false;
                    if(_FlashingMainColor == backColor)
                        CurrentLineBackColor = ChangedLineBackColor;
                    else CurrentLineBackColor = _FlashingMainColor;
                }
                else if (_FlashingIndex % 2 == 0)
                    CurrentLineBackColor = backColor;
                else
                    CurrentLineBackColor = _FlashingMainColor;
                _FlashingIndex++;
            };
            CurrentLineBackColor = backColor;
            _FlashingTimer.Interval = breaking;
            _FlashingTimer.Enabled = true;
        }
        public void Flashing(Color backColor, int milisecond = 1000, int breaking = 100)
        {
            Color _FlashingMainColor = BackColor;
            int _FlashingIndex = 0;
            Timer _FlashingTimer = new Timer();
            _FlashingTimer.Tick += (s, e) =>
            {
        ...
        private void DoDragDrop_old(Place place, string text)
        {
            ...
            if (draggedRange.ColumnSelectionMode)
            {
                ...
                insertRange = new Range(this, place, new Place(place.CharIndex, place.LineIndex + draggedRange.End.LineIndex - draggedRange.Start.LineIndex)) { ColumnSelectionMode = true };
                for (int i = LinesCount; i <= insertRange.End.LineIndex; i++)
                {
                    Selection.GoLast(false);
                    InsertChar('\n');
                }
            ...
            if (!insertRange.ReadOnly)
            {
                ...
                else
                {
                    ...
                    InsertText(text);
                    caretPositionAfterInserting = Selection.Start;
                    var lineLength = this[caretPositionAfterInserting.LineIndex].Count;
                    ...
                    var shift = lineLength - this[caretPositionAfterInserting.LineIndex].Count;
                    caretPositionAfterInserting.CharIndex = caretPositionAfterInserting.CharIndex - shift;
                    place.CharIndex = place.CharIndex - shift;
                }
                ...
                else
                {
                    ...
                    Selection = new Range(this, place,
                                            new Place(place.CharIndex + draggedRange.End.CharIndex - draggedRange.Start.CharIndex,
                                                    place.LineIndex + draggedRange.End.LineIndex - draggedRange.Start.LineIndex)) { ColumnSelectionMode = true };
                }
            }
                ...
                if (!draggedRange.Contains(place))
                {
                    ...
                    if ((place > draggedRange.Start) && (copyMode == false))
                    {
                        if (draggedRange.ColumnSelectionMode == false)
                        {
                            ...
                            if (dR.Start.LineIndex != dR.End.LineIndex) // If more then one line was selected/dragged ...
                            {
                                tS_S_Char = (dR.End.LineIndex != tP.LineIndex)
                                    ? tP.CharIndex
                                    : dR.Start.CharIndex + (tP.CharIndex - dR.End.CharIndex);
                                tS_E_Char = dR.End.CharIndex;
                            }
                            else // only one line was selected/dragged
                            {
                                if (dR.End.LineIndex == tP.LineIndex)
                                {
                                    tS_S_Char = tP.CharIndex - dR.Text.Length;
                                    tS_E_Char = tP.CharIndex;
                                }
                                else
                                {
                                    tS_S_Char = tP.CharIndex;
                                    tS_E_Char = tP.CharIndex + dR.Text.Length;
                                }
                            }
                            ...
                            if (dR.End.LineIndex != tP.LineIndex)
                            {
                                tS_S_Line = tP.LineIndex - (dR.End.LineIndex - dR.Start.LineIndex);
                                tS_E_Line = tP.LineIndex;
                            }
                            else
                            {
                                tS_S_Line = dR.Start.LineIndex;
                                tS_E_Line = dR.End.LineIndex;
                            }
                    ...
                    {
                        if ((copyMode == false) &&
                            (place.LineIndex >= dR.Start.LineIndex) && (place.LineIndex <= dR.End.LineIndex) &&
                            (place.CharIndex >= dR.End.CharIndex))
                        {
                            tS_S_Char = tP.CharIndex - (dR.End.CharIndex - dR.Start.CharIndex);
                            tS_E_Char = tP.CharIndex;
                        }
                        else
                        {
                            tS_S_Char = tP.CharIndex;
                            tS_E_Char = tP.CharIndex + (dR.End.CharIndex - dR.Start.CharIndex);
                        }
                        tS_S_Line = tP.LineIndex;
                        tS_E_Line = tP.LineIndex + (dR.End.LineIndex - dR.Start.LineIndex);
        ...
        #region Others
        public virtual void DelayedSelectionChanged(Range range)
        {
            if (range == null) range = Selection;
            if (DetectSameWords)
            {
                VisibleRange.ClearStyle(SameWordsStyle);
                if (!range.IsEmpty) return;//user selected diapason
                //get fragment around caret
                string text = range.GetFragment(@"\w",RegexOptions.IgnoreCase).Text;
                if (text.Length == 0) return;
                //highlight same words
                var ranges = VisibleRange.GetRanges("\\b" + text + "\\b").ToArray();
                if (ranges.Length > 1)
                    foreach (var r in ranges)
                        r.SetStyle(SameWordsStyle);
            }
        }
        public virtual void DelayedTextChanged(Range range)
        {
            if (range == null) return;
            if (DetectSecondLanguage)
            {
                Highlighter sh = new Highlighter(SecondLanguage);
                foreach (var r in this.GetRanges(SecondLanguagePattern, RegexOptions.Singleline))
                {
            ...
            if (DetectHyperLink)
            {
                range.ClearStyle(HyperLinkStyle);
                range.SetStyle(HyperLinkStyle, HyperLinkPattern);
            }
        }

        public bool DetectSameWords { get; set; } = true;
        public MarkerStyle SameWordsStyle = new MarkerStyle(new SolidBrush(Color.FromArgb(90, 128,128,128)));

        public bool DetectSecondLanguage { get; set; } = true;
        public Language SecondLanguage { get; set; } = Language.HTML;
        public string SecondLanguagePattern { get; set; } = @"``";


        public bool DetectHyperLink { get; set; } = true;
        public TextStyle HyperLinkStyle = new TextStyle(Brushes.Blue, null, FontStyle.Underline);
        public string HyperLinkPattern = @"(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?";

        public bool IsHyperlink(Place place)
        {
            var mask = GetStyleIndexMask(new Style[] { HyperLinkStyle });
            if (place.CharIndex < GetLineLength(place.LineIndex))
    ...
    public enum HighlightingRangeType
    {
        ...
        /// <summary>
        /// Highlight a scope range of text. Middle performance.
        /// </summary>
        ScopeRange,
    ...
    /// <summary>
    /// Strategy of search of end of block
    /// </summary>
